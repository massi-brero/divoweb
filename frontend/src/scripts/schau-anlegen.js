...
function renderJudges() {
  ...
  state.judges.forEach((judge, index) => {
    ...
    const associationSelect = fragment.querySelector('[data-judge-field="association"]')
...
async function onSubmit(event) {
  ...
  try {
    ...
    showToast('Die Schau konnte nicht gespeichert werden. Bitte prüfen Sie Backend und Eingaben.', 'error')
  } finally {
...
function onStartManagement() {
  if (!state.saved) {
    showToast('Bitte speichern Sie zuerst die Schaudaten.', 'error')
    return
  }

  window.location.href = 'zuechter-voegel.html'
}

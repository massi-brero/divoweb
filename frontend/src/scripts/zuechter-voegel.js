...
import { lookupVogelklasseMock } from '../services/vogelklasseService.js'
...
function bindBreederEvents() {
  ...
  document
    ...
    .addEventListener('click', createSampleData)
...
async function searchBreederByVerbandsnummer() {
  ...
  const verbandsnummer = document.getElementById('searchVerbandsnummer').value.trim()
  ...
  try {
    ...
    showToast('Die Züchtersuche ist fehlgeschlagen. Bitte Backend prüfen.', 'error')
  }
}
...
async function saveBreeder(event) {
  ...
  if (Object.keys(errors).length > 0) {
    showToast('Bitte prüfen Sie die Pflichtfelder im Züchterformular.', 'error')
    return
  }
  ...
  try {
    ...
    document.getElementById('breederSearchHint').textContent = 'Züchter wurde gespeichert.'
    showToast('Züchter wurde gespeichert.', 'success')
  } catch (error) {
    console.error('Fehler beim Speichern des Züchters:', error)
    showToast('Der Züchter konnte nicht gespeichert werden.', 'error')
  }
}
...
function readBreederForm() {
  return {
    ...
    katalogEinverstaendnis: document.getElementById('katalogEinverstaendnis').checked,
  }
}
...
function fillBreederForm(breeder) {
  ...
  document.getElementById('verbandsnummer').value = breeder.verbandsnummer ?? ''
...
function resetBreederForm() {
  ...
  document.getElementById('selectedBreederName').textContent = 'Noch kein Züchter ausgewählt'
...
function selectBreeder(breeder) {
  ...
  document.getElementById('selectedBreederName').textContent = getBreederDisplayName(breeder)
...
async function determineColor() {
  const skl1 = document.getElementById('skl1').value.trim()
  const skl2 = document.getElementById('skl2').value.trim()
  const skl3 = document.getElementById('skl3').value.trim()
  ...
  const vogelklasse = await lookupVogelklasseMock(skl1, skl2, skl3)

  document.getElementById('farbbezeichnung').value = vogelklasse.farbbezeichnung

  if (vogelklasse.id) {
    document.getElementById('vogelklasseId').value = vogelklasse.id
  }

  document.getElementById('vogelklasseId').focus()
}
...
async function saveBird(event) {
  ...
  if (!vogelklasseId) {
    showToast('Bitte eine Vogelklasse-ID erfassen. Später kommt sie aus SKL 1/2/3.', 'error')
    return
  }
  ...
  try {
    ...
    showToast('Der Vogel konnte nicht gespeichert werden.', 'error')
  }
}
...
function createSampleData() {
  ...
  document.getElementById('searchVerbandsnummer').value = sampleBreeder.verbandsnummer
  ...
  document.getElementById('skl2').value = '5'
  document.getElementById('skl3').value = '1'
  document.getElementById('vogelklasseId').value = '151'
  document.getElementById('farbbezeichnung').value = 'Normal hellgrün'
  ...
  state.birds = [
    {
      ...
      farbbezeichnung: 'Normal hellgrün',
    ...
    },
    {
      ...
      farbbezeichnung: 'Normal dunkelgrün',
    ...
    },
    {
      ...
      farbbezeichnung: 'Opalin hellblau',
  ...
  renderBirds()
  showToast('Testdaten wurden angelegt.', 'success')
}
...
function getBreederDisplayName(breeder) {
  return breeder.vollname || `${breeder.vorname ?? ''} ${breeder.nachname ?? ''}`.trim()
}
